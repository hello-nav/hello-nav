...
import ContainWrap from '@/components/Contain'
...
const genFilterByList =
  (list: (AppItem | CateItem)[]) => (filterKey: string) => {
    if (globalThis?.localStorage.__CATEGORY_TYPE__ === 'list') {
...
const toggleType = (setType: any, setList: any, type?: CategoryType) => {
  ...
  if (globalThis?.localStorage) {
    globalThis.localStorage.__CATEGORY_TYPE__ = type
  }
  setType(type)
  if (oldFilterKey) {
...
function App() {
  ...
  const { __CATEGORY_TYPE__ } = globalThis?.localStorage || {}
  const [type, setType] = useState<CategoryType>(
    __CATEGORY_TYPE__ || CATEGORY_TYPES[0],
  )
  if (!__CATEGORY_TYPE__ && globalThis?.localStorage) {
    globalThis.localStorage.__CATEGORY_TYPE__ = type
  }
