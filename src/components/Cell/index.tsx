...
function onAppClick(appItem: AppItem) {
...
function getImgSrc(fileName: string): string {
  return require(`../../images/icons/${fileName}`).default
}
...
const Cell = (appItem: AppItem & { title: string | undefined; isSettingMode: boolean }) => {
  ...
  const isFavoriteApp =
    ...
    !appItem.favorite &&
    favoriteAppNames.includes(appItem.name)
  ...
  return !isFavoriteApp ? (
    <li className={`cell ${isHiddenApp ? 'hide' : ''} ${appItem.favorite ? 'favorite' : ''}`}>
      <a className="app" href={homepage} title={name} onClick={() => onAppClick(appItem)}>
